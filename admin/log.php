...
if($search != ''){
$pagesql = "SELECT COUNT(id) AS total FROM `log` WHERE (`logger` LIKE '%".$search."%') OR (`post` LIKE '%".$search."%') OR (`type` LIKE '%".$search."%')"; 
} else {
$pagesql = "SELECT COUNT(id) AS total FROM `log`"; 
}
...
// Set Main Var
if($search != ''){
$newssql = "SELECT * FROM `log` WHERE (`logger` LIKE '%".$search."%') OR (`post` LIKE '%".$search."%') OR (`type` LIKE '%".$search."%') ORDER BY id DESC LIMIT $start_from, ".$results_per_page;
$newsresult = mysqli_query($rlink, $newssql);
} else {
$newssql = "SELECT * FROM `log` ORDER BY id DESC LIMIT $start_from, ".$results_per_page;
$newsresult = mysqli_query($rlink, $newssql);
}
?>
<!DOCTYPE html>
...
echo '<h1>The Log</h1></br>';
    echo '<form action="" method="GET">
        <input type="text" name="search" id="search"/>
        <input type="submit" value="Search" class="btn btn-primary" id=""/>
    </form>';
echo $pagerow["total"]." Logs";
if($newsresult->num_rows > 0) {
echo '<table id="newsfeed">';
echo '<tr><th>Logger</th><th>Post</th><th>Type</th><th>Time</th></tr>';
    // output data of each row
    while($newsrow = $newsresult ->fetch_assoc()) {
$lastlogin = new DateTime($newsrow["time"]);
  echo '<tr>
<td><b>'.$newsrow["logger"].'</b></td><td>'.$newsrow["post"].'</td><td>'.$newsrow["type"].'</td><td>'.$lastlogin->format( 'h:i  A m/d/Y' ).'</td></tr>';
}
echo '</table>';
}
echo '</br>';
for ($i=1; $i<=$total_pages; $i++) { 
    echo "<a href='log.php?page=".$i."&search=".$search."'><div class='pagesnav'>".$i."</div></a>"; 
};
}
if($debug == 1){
echo '<pre>';
