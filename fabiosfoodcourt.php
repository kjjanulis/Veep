...
// Set Main Var
$allitems = "SELECT * FROM `items` WHERE shop = 'Fabios Food Court' ORDER BY rand(" . date("Ymdh") . ") LIMIT $start_from, ".$results_per_page;
...
echo '<img src="https://veeple.online/Images/fabiosfoodcourt.png" alt="Fabios Food Court" class="headerlogo"></br>';
?>
<div class="stats" style="background-color: #e1d4b2;">
<?php
if ($allitemsresult->num_rows > 0) {
    // output data of each row
    while($row = $allitemsresult->fetch_assoc()) {
  echo '<div class="item" onclick="item('.$user.','.$row["id"].','.$row["cost"].','.$row["durability"].','.$row["consumable"].')"><img src="'.$row["image_link"].'" alt="'.$row["item_name"].'"></br><b>'.$row["item_name"].'</b></br>';
   echo $row["cost"].' Veep Coin';
